plot4 <- function() {
        ...
        # 2.2 convert the combined field into and appropriate data type/class
        hhPC$dateMinutes <- strptime(hhPC$dateMinutes, "%d/%m/%Y %H:%M:%S")
        
        ## Goal 3: create the set of 4 plots and save it in a png file
        png("./data/EDA_HH_PowerConsumption/plot4.png")
        par(mfrow = c(2,2))
        
        # 3.1 Global Active Power plot 1
        plot(hhPC2[,10], hhPC2[,3], type = "l", xlab = "", 
             ylab = "Global Active Power")
        
        # 3.2 Voltage plot 2
        plot(hhPC2[,10], hhPC2[,5], type = "l", xlab = "datetime",
                ylab = "Voltage")
        
        # 3.3 Energy sub metering plot 3
        plot(hhPC2[,10], hhPC2[,7], type = "l", xlab = "", 
             ylab = "Energy sub metering")
        lines(hhPC2[,10], hhPC2[,8], col = "red")
        lines(hhPC2[,10], hhPC2[,9], col = "blue")
        legend("topright", 
               legend = c("Sub_metering_1","Sub_metering_2","Sub_metering_3"), 
               col = c("black", "red", "blue"), lty = c(1,1,1),
               bty = "n", x.intersp = 0.75, y.intersp = 0.75)
        
        # 3.4 Global_reactive_power plot 4
        plot(hhPC2[,10], hhPC2[,4], type = "l", xlab = "datetime", 
